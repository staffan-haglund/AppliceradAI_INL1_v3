...
def package_generator(file='lagerstatus-500.csv'):
...
def fitness(solution):
    ...
    for item_id, knapsack in solution.items():
        if knapsack > 0:  # If assigned
            item = next(item for item in packages if item["id"] == item_id)
...
packages = list(package_generator(file="lagerstatus-500.csv"))
